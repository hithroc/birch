...
execute vid (CardRequest msg) = do
    ...
    unless (null filtCards') $ do
        ...
        let retmsg = Message 0 vid (unlines . map (\(tags, x) -> (if S.member PrintText tags || null pattachments then uncurry printCard else const "") x) $ (filter (not . isNotFound . snd . snd) filtCards)) (pattachments ++ aattachments) []
        sendMessage retmsg
